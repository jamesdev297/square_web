...
class AddUserSquarePage extends StatefulWidget with HomeWidget {
  Function(int) onNext;

  AddUserSquarePage({Key? key, required this.onNext}) : super(key: key);

  @override
  String pageName() => "AddSquarePage";

  @override
  MenuPack get getMenuPack => MenuPack(
    centerMenu: Text(L10n.ai_01_create_ai_square, style: centerTitleTextStyle),
  );

  @override
  _AddUserSquarePageState createState() => _AddUserSquarePageState();
...
class _AddUserSquarePageState extends State<AddUserSquarePage> {
  ...
  late UserSquareData originData;
  UserSquareData updatedData = UserSquareData(squareName: "");
  ...
  void initState() {
    ...
    onRandomize();
    updatedData.lang = DeviceUtil.browserInfo.language ?? "en";
    originData = UserSquareData.fromData(updatedData);
    
    // widget.selectAiPlayerBloc.add(LoadAiPlayerEvent(updatedData.aiPlayerId!));
  }
  ...
  void onSave() async {
    ...
    String? squareId = await SquareManager().addUserSquare(updatedData);
    if(squareId == null) {
      FullScreenSpinner.hide();
      return;
    }
    ...
    /*if(updatedData.tempSquareImgData == null) {
      updatedData.tempSquareImgData = await HttpResourceUtil.downloadBytes(updatedData.squareImgUrl!);
    }*/

    await SquareManager().uploadThumbnailSquare(squareId, updatedData.tempSquareImgData!);
    ...
    SquareModel? squareModel = await SquareManager().getSquare(squareId);
    FullScreenSpinner.hide();
    if(squareModel == null) {
      return;
    }
