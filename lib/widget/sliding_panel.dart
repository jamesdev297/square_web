...
class SlidingPanel extends StatefulWidget {
  ...
  final ShapeDecoration? panelShapeClipDecoration;
  ...
  SlidingPanel({
    ...
    this.header,
    this.footer, this.panelShapeClipDecoration
...
class _SlidingPanelState extends State<SlidingPanel> with SingleTickerProviderStateMixin{
  ...
  VelocityTracker _vt = new VelocityTracker.withKind(PointerDeviceKind.mouse);
  ...
  void initState(){
    ...
    ac = AnimationController(
      ...
      if(widget.onPanelSlide != null) widget.onPanelSlide!(ac!.value);

      if(widget.onPanelOpened != null && ac!.value == 1.0) widget.onPanelOpened!();

      if(widget.onPanelClosed != null && ac!.value == 0.0) widget.onPanelClosed!();
    });
      ...
      children: <Widget>[
        ...
        widget.body != null ? AnimatedBuilder(
          ...
          builder: (context, child){
            return Positioned(
              top: !_isHorizontal ? (widget.parallaxEnabled ? _getParallax() : 0.0) : null,
              left: _isHorizontal ? (widget.parallaxEnabled ? _getParallax() : 0.0) : null,
              child: child!,
            );
          },
          child: Container(
        ...
        !widget.backdropEnabled ? Container() : GestureDetector(
          onVerticalDragEnd: widget.backdropTapClosesPanel ? (DragEndDetails dets){
            ...
            if((_isForward ? 1 : -1) * dets.velocity.pixelsPerSecond.dy > 0)
              _close();
          } : null,
          ...
          child: AnimatedBuilder(
              ...
              builder: (context, _) {
                return Container(
                  ...
                  color: ac!.value == 0.0 ? null : widget.backdropColor.withOpacity(widget.backdropOpacity * ac!.value),
                );
              }
          ),
        ),
        ...
        !_isPanelVisible ? Container() : _gestureHandler(
          child: AnimatedBuilder(
            ...
            builder: (context, child) {
              return Container(
                height: !_isHorizontal ? ac!.value * (widget.maxSize - widget.minSize) + widget.minSize : null,
                width: _isHorizontal ? ac!.value * (widget.maxSize - widget.minSize) + widget.minSize : null,
                margin: widget.margin,
                padding: widget.padding,
                decoration:
                widget.panelShapeClipDecoration != null ? widget.panelShapeClipDecoration :
                widget.renderPanelSheet ? BoxDecoration(
                  border: widget.border,
                  borderRadius: widget.borderRadius,
                  boxShadow: widget.boxShadow,
                  color: widget.color,
                ) : null,
                clipBehavior: widget.panelShapeClipDecoration != null ? Clip.antiAlias : Clip.none,
                child: child,
              );
            },
            child: Stack(
              ...
              children: <Widget>[
                ...
                Positioned(
                    ...
                    right: widget.slideDirection == SlideDirection.RIGHT ? 0.0 : null,
                    width: _isHorizontal ? ac!.value * (widget.maxSize - widget.minSize) + widget.minSize : null,
                        /*MediaQuery.of(context).size.width - (_isHorizontal ?
                        (widget.margin != null ? widget.margin!.horizontal : 0) -
                        (widget.padding != null ? widget.padding!.horizontal : 0) : 0),*/
                    height: MediaQuery.of(context).size.height - (!_isHorizontal ?
                        (widget.margin != null ? widget.margin!.vertical : 0) -
                        (widget.padding != null ? widget.padding!.vertical : 0) : 0),
                    child: Container(
                      height: !_isHorizontal ? widget.maxSize : null,
                      width: _isHorizontal ? widget.maxSize : null,
                ...
                Positioned(
                  ...
                  child: Container(
                    height: !_isHorizontal ? widget.minSize : null,
                    width: _isHorizontal ? widget.minSize : null,
                    child: widget.collapsed == null ? Container() : FadeTransition(
                      opacity: Tween(begin: 1.0, end: 0.0).animate(ac!),
  ...
  double _getParallax(){
    if(_isForward)
      return -ac!.value * (widget.maxSize - widget.minSize) * widget.parallaxOffset;
    else
      return ac!.value * (widget.maxSize - widget.minSize) * widget.parallaxOffset;
  }
  ...
  Widget _gestureHandler({Widget? child}){
    if (!widget.isDraggable) return child ?? Container();

    if (widget.panel != null){
      return GestureDetector(
        onVerticalDragUpdate: !_isHorizontal ? (DragUpdateDetails dets) => _onGestureSlide(dets.delta.dy) : null,
        onVerticalDragEnd: !_isHorizontal ? (DragEndDetails dets) => _onGestureEnd(dets.velocity) : null,
        onHorizontalDragUpdate: _isHorizontal ? (DragUpdateDetails dets) => _onGestureSlide(dets.delta.dx) : null,
        onHorizontalDragEnd: _isHorizontal ? (DragEndDetails dets) => _onGestureEnd(dets.velocity) : null,
        child: child,
      );
    }
    ...
    return Listener(
      ...
      onPointerMove: (PointerMoveEvent p){
        ...
        _onGestureSlide(_isHorizontal ? p.delta.dx : p.delta.dy);
      },
  ...
  void _onGestureSlide(double delta){

    // only slide the panel if scrolling is not enabled
    if(!_scrollingEnabled){
      if(_isForward)
        ac!.value -= delta / (widget.maxSize - widget.minSize);
      else
        ac!.value += delta / (widget.maxSize - widget.minSize);
    }
  ...
  void _onGestureEnd(Velocity v){
    ...
    double visualVelocity = (_isHorizontal ? -v.pixelsPerSecond.dx : -v.pixelsPerSecond.dy) / (widget.maxSize - widget.minSize);
    ...
    double d2Close = ac!.value;
    double d2Open = 1 - ac!.value;
    double d2Snap = ((widget.snapPoint ?? 3) -ac!.value).abs(); // large value if null results in not every being the min
    double minDistance = min(d2Close, min(d2Snap, d2Open));

    // check if velocity is sufficient for a fling
    if((_isHorizontal ? v.pixelsPerSecond.dx : v.pixelsPerSecond.dy).abs() >= minFlingVelocity){

      // snapPoint exists
      if(widget.panelSnapping && widget.snapPoint != null){
        if((_isHorizontal ? v.pixelsPerSecond.dx : v.pixelsPerSecond.dy).abs() >= kSnap*minFlingVelocity
            || minDistance == d2Snap)
  ...
  Future<void> _hide(){
    return ac!.fling(velocity: -1.0).then((x){
      setState(() {
  ...
  Future<void> _animatePanelToPosition(double value, {Duration? duration, Curve curve = Curves.linear}){
    ...
    return ac!.animateTo(value, duration: duration, curve: curve);
  }
  ...
  Future<void> _animatePanelToSnapPoint({Duration? duration, Curve curve = Curves.linear}){
    ...
    return ac!.animateTo(widget.snapPoint!, duration: duration, curve: curve);
  }
  ...
  double get _panelPosition => ac!.value;

  //returns whether or not
  //the panel is still animating
  bool get _isPanelAnimating => ac!.isAnimating;

  //returns whether or not the
  //panel is open
  bool get _isPanelOpen => ac!.value == 1.0;

  //returns whether or not the
  //panel is closed
  bool get _isPanelClosed => ac!.value == 0.0;
...
class PanelController{
  ...
  Future<void> animatePanelFromCurrentPosition(double value, {Duration? duration, Curve curve = Curves.linear}){
    assert(isAttached, "PanelController must be attached to a SlidingUpPanel");
    assert(-1.0 <= value && value <= 1.0);
    if(value >= 0)
      value = min(1, value + _panelState!._panelPosition);
    else if(value < 0)
      value = max(0, value + _panelState!._panelPosition);
  ...
  AnimationController? get animationController => _panelState?.ac;
